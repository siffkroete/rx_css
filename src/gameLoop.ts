...
const update = (deltaTime: number, state: any, inputState: any): any => {
  ...
  if(state['objects'] === undefined) {
    ...
    state['objects'].forEach((obj: { isPaused: boolean; toggleColor: any; color: any; x: number; velocity: { x: number; y: number; }; y: number; }) => {
      // Process Inputs
...
const render = (state: any) => {
  ...
  state['objects'].forEach((obj: { color: string | CanvasGradient | CanvasPattern; x: number; y: number; width: number; height: number; }) => {
...
const calculateStep: (prevFrame: IFrameData) => Observable<IFrameData> = (prevFrame: IFrameData) => {
  return Observable.create((observer: { next: (arg0: { frameStartTime: number; deltaTime: number; }) => void; }) => {
...
const frames$ = of(undefined)
  .pipe(
    expand((val: any) => calculateStep(val)),
    // Expand emits the first value provided to it, and in this
    //  case we just want to ignore the undefined input frame
    filter((frame: any) => frame !== undefined),
...
frames$
  .pipe(
    ...
    tap((gameState: any) => gameState$.next(gameState))
